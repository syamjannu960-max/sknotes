...
import { enrichChapterContent } from "@/ai/flows/enrich-chapter-content";
...
type ChapterFormProps = {
  ...
  defaultValue?: ChapterFormValues;
...
export function ChapterForm({
  ...
  const handleEnrichContent = async () => {
    ...
    setSuggestions('');
    try {
      const result = await enrichChapterContent({ chapterText: currentDescription });
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl">
        <DialogHeader>
          <DialogTitle>{defaultValue?.id ? "Edit Chapter" : "Add New Chapter"}</DialogTitle>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 max-h-[80vh] overflow-y-auto p-1 pr-4">
            ...
            <FormField control={form.control} name="description" render={({ field }) => (
                <FormItem>
                    ...
                    <FormControl><Textarea placeholder="Provide a detailed description or syllabus for the chapter." {...field} rows={8} /></FormControl>
                    <FormMessage />
                </FormItem>
            )} />
            {suggestions && (
            ...
            <FormField control={form.control} name="pdfUrl" render={({ field }) => (
                <FormItem>
                    ...
                    <FormControl><Input placeholder="https://example.com/chapter.pdf" {...field} /></FormControl>
                    <FormDescription>Link to a publicly accessible PDF file for this chapter.</FormDescription>
                    <FormMessage />
                </FormItem>
            )} />
